...
function applyZoom(newZoom, anchor = null, onDone = null) {
    ...
    function renderAllPagesForDoc(pdfDoc, baseWidth, onFirstPageReady) {
        ...
        // Build the new placeholders + first page in a DETACHED fragment.
        // The old canvases stay untouched and fully visible in the live
        // container while this happens -- nothing is cleared or resized
        // yet, so there is zero flicker during the wait. We only touch the
        // real DOM once the new page 1 canvas has actually finished
        // rendering (see swapInNewContent below), and we do the container
        // resize + content swap + scroll fix in that same moment so it's
        // a single atomic repaint instead of a resize-then-pop-in.
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            newContent.appendChild(placeholder);
            placeholders.push(placeholder);
        }

        function renderPageInto(placeholder, pageNum) {
            if (placeholder.dataset.rendered === 'true') return Promise.resolve();
        ...
        function attachLazyLoadingForRest() {
        ...
        function swapInNewContent() {
            ...
            attachLazyLoadingForRest();
            if (onFirstPageReady) onFirstPageReady();
        }

        if (placeholders[0]) {
            renderPageInto(placeholders[0], 1).then(swapInNewContent).catch(swapInNewContent);
        } else {
            swapInNewContent();
        }
    }
